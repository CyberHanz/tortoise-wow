...
delayedPackets ChatReplyAction::GenerateResponsePackets(const std::string json
    , const WorldPacket chatTemplate, const WorldPacket emoteTemplate, const WorldPacket systemTemplate, const std::string startPattern, const std::string endPattern, const std::string deletePattern, const std::string splitPattern, bool debug, const std::string& fallbackText, const std::set<std::string>& itemQualifiers)
{
    ...
    // Ticket 4a debug fix: deterministic clickable-itemlink post-processing,
    // applied to the model's own reply lines only (never to debugLines,
    // which are diagnostic echoes of the prompt/raw response, not chat
    // output) -- see ChatHelper::LinkifyItemMentions()'s comment for why
    // this happens here (after ParseResponse(), before LinesToPackets())
    // and how it guarantees the LLM never has to emit |Hitem:...|h itself.
    // No-op (itemQualifiers empty) reproduces the exact previous behaviour.
    if (!itemQualifiers.empty())
    {
        for (std::string& line : lines)
            line = ChatHelper::LinkifyItemMentions(line, itemQualifiers);
    }
        ...
        )
    {
        ...
        if (player)
        {
            ...
            if (player != bot && (IsRealPlayer(player) || (sPlayerbotAIConfig.llmBotToBotChatChance && urand(0, 99) < sPlayerbotAIConfig.llmBotToBotChatChance)))
            {
                ...
                // Cleanup pass (2026-09-08): general grounding rules, same
                // unconditional "<pre prompt>" placement and same reasoning
                // as the language-framing block just above -- these apply to
                // every real-player turn, not just item-related ones, so
                // they live here rather than inside any conditional block
                // below. Deliberately narrow, additive sentences (no
                // rewrite of the block above) so this stays a minimal diff:
                // (1) no invented past actions/events/emotions-as-fact/
                // provenance ("I heard from X that...") beyond what this
                // prompt or the character description actually states;
                // (2) don't contradict a factual self-statement the current
                // speaker just made unless authoritative data here says
                // otherwise (mirrors the existing item-data "authoritative"
                // framing, generalised to any in-prompt fact);
                // (3) canonical WoW proper nouns (race/class/faction/place
                // names) are never translated/reworded, even when replying
                // in Dutch -- otherwise a Dutch reply calling a High Elf a
                // "Hoge Elf" or Ironforge "IJzerhold" reads as invented/
                // wrong lore to a player who knows the English canon name;
                // (4) an item name in brackets that is NOT covered by an
                // <item data> block elsewhere in this prompt (see Ticket 4a
                // below) has no real information behind it -- covers a
                // message with a plain "[Item Name]" mention that never
                // resolved to a real itemlink, so BuildItemContextBlock()
                // below produced nothing for it.
                jsonFill["<pre prompt>"] += " Do not invent specific past actions, events, or emotions as fact, and do not cite a source for what you know (e.g. \"I heard from X that...\") unless it is explicitly given to you in this prompt or your character description. "
                    "Do not contradict a factual statement " + playerName + " just made about themselves unless you have authoritative data in this prompt that says otherwise. "
                    "Never translate, reword, or Dutchify canonical World of Warcraft proper nouns -- race, class, faction, and place names such as High Elf, Night Elf, Blood Elf, Ironforge, Stormwind, etc. always keep their exact English name, even when the rest of your reply is in Dutch. "
                    "If an item name appears in brackets in the message below but is not backed by an item data block in this prompt, you have no real information about that item -- do not describe its stats, uses, or effects.";
                ...
                // Ticket 4a debug fix: the block itself already only ever
                // contains fields that really exist on the resolved
                // ItemPrototype (see BuildItemContextBlock()'s own
                // comment) -- the live bug was never missing data, it was
                // that a single "don't invent extra things" sentence never
                // told the model it MUST use a field that IS present
                // instead of hedging with "I don't know", the same failure
                // mode Ronde 11 already fixed once for persistent memory
                // (see that comment above). So this now states both
                // directions explicitly: use what's listed, directly and
                // without hedging; treat anything not listed as simply not
                ...
                // Grounding follow-up (2026-09-08): the pre-prompt rules
                // above (current-speaker framing, no-invented-lore, no-
                // contradiction, canonical-name) sit in "<pre prompt>",
                // separated from the actual generation point by up to
                // llmContextLength characters of "<context>" plus
                // "<prompt>" itself -- per the raw-completion template
                // "[<pre prompt>]<context> <prompt> <post prompt>"
                // (PlayerbotAIConfig.cpp). Proven live to be too weak: bots
                // still contradicted a player's own statement and invented
                // lore despite those rules. This is a short, per-call
                // reminder built fresh here (not the static config
                // default) and placed immediately before the existing
                // "<sender name>:" cue -- the closest text to generation,
                // so it carries the most weight for a small/local model.
                // Deliberately short: it does NOT repeat the message
                // itself (already present just above via "<prompt>"), only
                // the highest-priority constraints, so it stays well under
                // the 100-character reply budget's worth of competing
                // instructions.
                jsonFill["<post prompt>"] = " Reminder: " + playerName + "'s statement about themselves just above is authoritative about their own current state unless authoritative data above says otherwise -- do not contradict it from assumption or lore. No invented actions, emotions, or lore. Keep canonical WoW names in English. " + sPlayerbotAIConfig.llmPostPrompt;
                ...
                futurePackets futPackets = std::async(std::launch::async, ChatReplyAction::GenerateResponsePackets, json, chatTemplate, emoteTemplate, systemTemplate, startPattern, endPattern, deletePattern, splitPattern, debug, memoryLlmFallbackText, itemQualifierStrings);
